...
[dataSet,fileNames] = create_data_set_MI(directory,fileIdx,[0,0.05,0.1,0.15,0.2],[650,1000]);
Names = string(fileNames);
for i = 1:length(Names)
    disp(strcat(num2str(i),': ',Names(i)));
end
...
for i = 1:length(dataSet)
    ...
    Rd = zeros(numel(dataSet{i}.Rd(yroi,xroi,j,1)),size(dataSet{i}.Rd(yroi,xroi,j,:),4));
        ...
        OPs = inverseHomogeneous(Rd');
        dataSet{i}.OP(:,:,j,1) = reshape(OPs(1,:),size(dataSet{i}.Rd(yroi,xroi,j,k)));
        dataSet{i}.OP(:,:,j,2) = reshape(OPs(2,:),size(dataSet{i}.Rd(yroi,xroi,j,k)));
    end
    progressbar(i/length(dataSet));
end
